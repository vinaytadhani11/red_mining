                                        ...
                                        DropdownButton(
                                          ...
                                          items: con.categoryList.map((element) {
                                            return DropdownMenuItem(
                                              onTap: () {
                                                con.selectCategory.value = element.id.toString();
  ...
  Widget ticketList() {
    return Obx(
      () => ListView.builder(
        ...
        itemCount: con.query.length,
        itemBuilder: (context, index) {
          final data = con.query[index];
          String formattedDate = DateFormat('dd/MM/yyyy | kk:mm a').format(DateTime.parse(data.createdAt ?? ""));
          return InkWell(
            onTap: () {
              con.selectCType.value = data.categoryName ?? "";
              con.selectStatus.value = data.status ?? "";
              // if (data.status == "0") {
              Get.to(() => TicketMessageScreen(queryID: data.id!));
              // } else if (data.status == "1") {
              // AppFunction.showSnackBar(title: "Error", message: "Ticket is Resolves");
              // } else {
              // AppFunction.showSnackBar(title: "Error", message: "Ticket is Close");
              // }
            },
            child: Container(
              width: Get.width,
                          ...
                          child: Text.rich(
                            TextSpan(
                              children: [
                                ...
                                TextSpan(text: formattedDate, style: TextStyle(fontSize: 14, color: Colors.white, fontWeight: FontWeight.bold)),
                              ],
                              // '21/08/2023  |  12:40 PM'
                            ),
                          ),
                          ...
                          child: Text.rich(
                            TextSpan(
                              children: [
                                TextSpan(
                                    text: 'Title\n', style: TextStyle(fontSize: 16, color: Colors.white, fontWeight: FontWeight.w500, height: 2)),
                                TextSpan(text: data.title ?? "", style: TextStyle(fontSize: 15, color: Colors.white, fontWeight: FontWeight.bold)),
                              ],
                            ),
                          ),
                        ),
                    ...
                    child: Text.rich(
                      TextSpan(
                        children: [
                          TextSpan(
                              text: 'Description\n', style: TextStyle(fontSize: 16, color: Colors.white, fontWeight: FontWeight.w500, height: 2)),
                          TextSpan(text: data.description ?? "", style: TextStyle(fontSize: 14, color: Colors.white, fontWeight: FontWeight.bold)),
                        ],
                      ),
                    ),
                  ),
